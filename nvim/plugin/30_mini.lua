...
-- Step one ===================================================================
-- Enable 'poimandres' color scheme.
--
-- See also:
...
now(function()
  MiniDeps.add({
    name = 'poimandres',
    source = 'file://' .. vim.fn.expand('~/dev/poimandres.nvim'),
  })
...
later(function()
  ...
  MiniKeymap.map_multistep(
    'i',
    '<Tab>',
    { 'minisnippets_expand', 'minisnippets_next', 'pmenu_next' }
  )
  ...
  snippets.setup({
    snippets = {
      ...
      function()
        ...
        local file = ({ typescriptreact = 'react', javascriptreact = 'react' })[ft]
          or ft
        local path = config_path .. '/snippets/' .. file .. '.json'
        if vim.fn.filereadable(path) == 1 then return snippets.read_file(path) end
        return {}
      end,
    },
