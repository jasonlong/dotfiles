...
now(function()
	...
	-- Detect macOS appearance and use storm variant for light mode
	local handle = io.popen("defaults read -g AppleInterfaceStyle 2>/dev/null")
	local result = handle and handle:read("*a") or ""
	if handle then handle:close() end
	local style = result:match("Dark") and "" or "storm"

	require("poimandres").setup({ style = style })
...
-- mini.snippets disabled — emmet-language-server handles abbreviation expansion
-- via LSP, and snippet items it returns are expanded by mini.completion through
-- vim.snippet.expand() (built-in to Neovim 0.10+).
